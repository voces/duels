 ...
 */
export function loadJsonFile(fname: string): IProjectConfig {
...
export function toArrayBuffer(b: Buffer): ArrayBuffer {
	...
	for (let i = 0; i < b.length; ++i) view[i] = b[i];
 ...
 *
 */
export function compileMap(config: IProjectConfig): boolean {
	if (!config.mapFolder) {
 ...
 */
const loggerFormatFunc = printf(
	({ level, message, timestamp }) =>
		`[${timestamp.replace("T", " ").split(".")[0]}] ${level}: ${message}`,
);
