...
export const updateMouse = (
	...
	const oldEffectiveTarget =
		mice[playerId].targetLock ?? mice[playerId].target;

	const newTarget =
		data.target === NONE ? null : data.target ?? mice[playerId].target;
	const newTargetLock =
		data.targetLock === NONE
			? null
